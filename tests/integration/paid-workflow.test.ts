...
describe("paid workflow (owner approves → accountant pays)", () => {
  ...
  it("advance: owner approves, accountant pays; manager cannot pay; debt counts paid", async () => {
    ...
    expect((await mgr.client.from("advances").update({ approval_status: "paid" }).eq("id", a!.id)).error).not.toBeNull();
    // Accountant marks paid
    expect((await acct.client.from("advances").update({ approval_status: "paid" }).eq("id", a!.id)).error).toBeNull();
    ...
    expect(row!.paid_by).toBe(acct.userId);
  ...
  it("expense: manager logs (pending) → owner approves → accountant pays", async () => {
    ...
    // Owner cannot mark paid (accountant-only)
    expect((await owner.client.from("consumables").update({ approval_status: "paid" }).eq("id", e!.id)).error).not.toBeNull();
    // Accountant marks paid
    expect((await acct.client.from("consumables").update({ approval_status: "paid" }).eq("id", e!.id)).error).toBeNull();
