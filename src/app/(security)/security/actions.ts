...
// Security registers material moving in/out at the gate.
export async function recordGateLog(formData: FormData): Promise<void> {
  const me = await getProfile();
  if (!me || (me.role !== "security" && me.role !== "owner")) return;
...
// Security acknowledges a manager/owner-issued gate pass before release.
export async function acknowledgeGatePass(formData: FormData): Promise<void> {
  const me = await getProfile();
  if (!me || me.role !== "security") return;
