    ...
    <main className="p-6 max-w-4xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Log arrivals in, send them to the plant, and authorise outgoing material.</p>
        </div>
        <Link href="/gate/intake" className="rounded bg-ore px-4 py-2 text-sm font-semibold text-white hover:bg-ore-strong">
          + New intake
        </Link>
      </header>

      {/* Arrivals dwelling at the gate */}
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold">At the gate (awaiting send-in)</h2>
            <Badge variant={atGate?.length ? "yellow" : "default"}>{atGate?.length ?? 0}</Badge>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          {(atGate?.length ?? 0) === 0 ? (
            <p className="px-4 py-3 text-sm text-ink-2">No supplier waiting at the gate.</p>
          ) : (
            <ul className="divide-y divide-line">
              {(atGate ?? []).map((v) => {
                const sup = g1<{ name: string }>((v as { supplier: unknown }).supplier);
                const mat = g1<{ name: string }>((v as { material: unknown }).material);
                return (
                  <li key={v.id as string} className="flex flex-wrap items-center justify-between gap-2 px-4 py-3 text-sm">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-ink-2">{mat?.name ?? "—"}{v.vehicle_plate ? ` · ${v.vehicle_plate}` : ""}</span>
                      <Badge variant="default">{v.entry_path as string}</Badge>
                    </div>
                    <form action={sendVisitIn}>
                      <input type="hidden" name="visit_id" value={v.id as string} />
                      <button type="submit" className="rounded bg-approve px-3 py-1 text-xs font-semibold text-white">
